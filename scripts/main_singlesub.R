...
model <- gsub("Mania", latentnames[2], model)
model <- gsub("SCZ", traitnames[1], model)
model <- gsub("BD", traitnames[2], model)
...
var1 <- round(model_LD$results[2, "STD_Genotype"], 2)
var2 <- round(model_LD$results[3, "STD_Genotype"], 2)
var3 <- round(model_LD$results[1, "STD_Genotype"], 2)
...
square_labels <- c(traitnames[1], traitnames[2])
circle_labels <- c(latentnames[1], latentnames[2])
arrow_labels <- c(var1, var2, var3)

squares <- data.frame(
  x = rep(c(1, 1 + x_distance), each = 4),
  y = rep(0, 8),
...
circles <- data.frame(
  x = rep(c(1, 1 + x_distance), each = 100),
  y = rep(square_size + y_distance, 200),
  angle = rep(seq(0, 2 * pi, length.out = 100), 2),
...
square_labels_df <- data.frame(
  x = c(1, 1 + x_distance),
  y = rep(-square_size/2 - 0.2, 2),
  label = square_labels
)

circle_labels_df <- data.frame(
  x = c(1, 1 + x_distance),
  y = rep(circle_radius + 2*y_distance + 0.2, 2),
  label = circle_labels
)
...
ggplot() +
  ...
  geom_segment(aes(x = 1, y = y_distance+circle_radius, xend = 1 + x_distance, yend = square_size/2), arrow = arrow(length = unit(0.2, "cm")), color = "black") +
  ...
  geom_text(aes(x = 2.4, y = 1.15, label = arrow_labels[3]), color = "black", vjust = -0.5) +
...
sub <- paste(latentnames[2], "~SNP", sep="")
...
data.table::fwrite(x = output[[1]],file = paste0(latentnames[2], "_gwas.gz"), append = F,quote = F,sep = "\t",col.names = T,nThread=20)
...
superm <- supermunge(filePaths = c(paste0(latentnames[2], "_gwas.gz")),
...
LDSCoutput<-ldsc(traits = c(files_input[1], output_name_gz), 
...
trait_names <- c(corr$trait, latentnames[2])
original_order <- c(latentnames[2], corr$trait)
...
R_mania <- S[latentnames[2], ]
R_bd <- S[traitnames[2], ]
seR_mania <- SE_R[latentnames[2], ]
seR_bd <- SE_R[traitnames[2], ]
...
colnames(merged_df)[2] <- paste0("Correlation_", latentnames[2])
colnames(merged_df)[3] <- paste0("Correlation_", traitnames[2])
colnames(merged_df)[4] <- paste0("SE_", latentnames[2])
colnames(merged_df)[5] <- paste0("SE_", traitnames[2])
...
lines[2] <- traitnames[2]
lines[3] <- latentnames[2]
